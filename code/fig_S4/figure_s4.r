pdf("Figure_S4.pdf",height=18,width=12)
#PSMC
par(mfrow=c(3,1))
hm_st <- read.table("NA12878.rm.ST.0.txt")
un_st <- read.table("NA12878.un.ST.0.txt")
hm_1000g <- read.table("NA12878.hm.1000g.0.txt")
un_1000g <- read.table("NA12878.un.1000g.0.txt")

par(mar = c(7, 7, 4, 4))
plot(un_st$V1/1000,un_st$V2,ylim=c(0,2.5),xlim=c(1,5000),log="x",type="n",lwd="3",xlab="",ylab="",axes=F,cex.main=1.75)

rect(0.1,2.3,127,6, col=scales::alpha(rgb(col2rgb("cyan")[1,],col2rgb("cyan")[2,],col2rgb("cyan")[3,],max = 255), 0.25),border="transparent")
text(127/2,2.4,"Holocene",cex=1.5,font=4)
rect(128,2.3,2580,6, col=scales::alpha(rgb(col2rgb("yellow")[1,],col2rgb("yellow")[2,],col2rgb("yellow")[3,],max = 255), 0.25),border="transparent")
text(2580/2,2.4,"Pleistocene",cex=1.5,font=4)
rect(2580,2.3,15330,6, col=scales::alpha(rgb(col2rgb("green")[1,],col2rgb("green")[2,],col2rgb("green")[3,],max = 255), 0.25),border="transparent")
text(3500,2.4,"Pliocene",cex=1.5,font=4)
abline(v=800,lwd=4,lty=3)
text(2000,0.1,"Ancient Time Period",cex=1.5,font=4)
abline(v=50,lwd=4,lty=3)
text(250,0.1,"Mid-Time Period",cex=1.5,font=4)
text(10,0.1,"Recent Time Period",cex=1.5,font=4)

lines(un_st$V1/1000,un_st$V2,type="s",lwd="3",col="red")
lines(hm_st$V1/1000,hm_st$V2,type="s",lwd="3",col="blue")
lines(un_1000g$V1/1000,un_1000g$V2,type="s",lwd="3",col="red")
points(un_1000g$V1/1000,un_1000g$V2,pch=18,cex=2,col="red")
lines(hm_1000g$V1/1000,hm_1000g$V2,type="s",lwd="3",col="blue")
points(hm_1000g$V1/1000,hm_1000g$V2,pch=18,cex=2,col="blue")
axis(1, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=1, line=3.2, "Time in past (KYA)", col="black", font=2,cex=1.75)
axis(2, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=2, line=3.2, expression(bold("Effective Population Size x 10"^4)),col="black", font=2,cex=1.75)
mtext(side=3, line=1.5, "A",col="black", at=1,font=2,cex=1.75)
box()
legend("topleft", inset=0.2, legend=c("Human PSMC Unmasked","Human PSMC Masked"),fill=c("red","blue"),cex=1.5,text.font=2)
legend(15,1.5, inset=0.3, legend=c("Samtools","1000g"),pch=c(NA,18),lty=c(1,1),lwd=3,cex=1.5,text.font=2)
#SMCPP
ceu_hm <- read.csv("smcpp_hm.csv")
ceu_un <- read.csv("smcpp_un.csv")

par(mar = c(7, 7, 4, 4))
plot(ceu_un$x/1000,ceu_un$y/10000,xlim=c(1,5000),log="x",type="n",lwd="3",xlab="",ylab="",axes=F,cex.main=1.7,ylim=c(0,2.5))

rect(0.1,2.3,127,6, col=scales::alpha(rgb(col2rgb("cyan")[1,],col2rgb("cyan")[2,],col2rgb("cyan")[3,],max = 255), 0.25),border="transparent")
text(127/2,2.4,"Holocene",cex=1.5,font=4)
rect(128,2.3,2580,6, col=scales::alpha(rgb(col2rgb("yellow")[1,],col2rgb("yellow")[2,],col2rgb("yellow")[3,],max = 255), 0.25),border="transparent")
text(2580/2,2.4,"Pleistocene",cex=1.5,font=4)
rect(2580,2.3,15330,6, col=scales::alpha(rgb(col2rgb("green")[1,],col2rgb("green")[2,],col2rgb("green")[3,],max = 255), 0.25),border="transparent")
text(3500,2.4,"Pliocene",cex=1.5,font=4)
abline(v=800,lwd=4,lty=3)
text(2000,0.1,"Ancient Time Period",cex=1.5,font=4)
abline(v=50,lwd=4,lty=3)
text(250,0.1,"Mid-Time Period",cex=1.5,font=4)
text(10,0.1,"Recent Time Period",cex=1.5,font=4)

lines(ceu_un$x/1000,ceu_un$y/10000,type="s",lwd="3",col="red")
lines(ceu_hm$x/1000,ceu_hm$y/10000,type="s",lwd="3",col="blue")
axis(1, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=1, line=3.2, "Time in past (KYA)", col="black", font=2,cex=1.75)
axis(2, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=2, line=3.2, expression(bold("Effective Population Size x 10"^4)),col="black", font=2,cex=1.75)
mtext(side=3, line=1.5, "B",col="black", at=1,font=2,cex=1.75)
box()
legend("topleft", inset=0.2, legend=c("Human CEU SMC++ Unmasked","Human CEU SMC++ Masked"),fill=c("red","blue"),cex=1.5,text.font=2)

#Stairway plot
ceu_hm <- read.table("ceu_rm_unfold.final.summary",header=T)
ceu_un <- read.table("ceu_un_unfold.final.summary",header=T)


par(mar = c(7, 7, 4, 4))
plot(ceu_un$year/1000,ceu_un$Ne_median/10000,xlim=c(1,5000),ylim=c(0,3.3),log="x",type="n",lwd="3",xlab="",ylab="",axes=F,cex.main=1.75)

rect(0.1,2.9,127,6, col=scales::alpha(rgb(col2rgb("cyan")[1,],col2rgb("cyan")[2,],col2rgb("cyan")[3,],max = 255), 0.25),border="transparent")
text(127/2,3.1,"Holocene",cex=1.5,font=4)
rect(128,2.9,2580,6, col=scales::alpha(rgb(col2rgb("yellow")[1,],col2rgb("yellow")[2,],col2rgb("yellow")[3,],max = 255), 0.25),border="transparent")
text(2580/2,3.1,"Pleistocene",cex=1.5,font=4)
rect(2580,2.9,15330,6, col=scales::alpha(rgb(col2rgb("green")[1,],col2rgb("green")[2,],col2rgb("green")[3,],max = 255), 0.25),border="transparent")
text(3500,3.1,"Pliocene",cex=1.5,font=4)
abline(v=800,lwd=4,lty=3)
text(2000,0.1,"Ancient Time Period",cex=1.5,font=4)
abline(v=50,lwd=4,lty=3)
text(250,0.1,"Mid-Time Period",cex=1.5,font=4)
text(10,0.01,"Recent Time Period",cex=1.5,font=4)

lines(ceu_un$year/1000,ceu_un$Ne_median/10000,type="s",lwd="3",col="red")
lines(ceu_hm$year/1000,ceu_hm$Ne_median/10000,type="s",lwd="3",col="blue")

axis(1, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=1, line=3.2, "Time in past (KYA)", col="black", font=2,cex=1.75)
axis(2, col="black",las=1,font=2,font.axis=4,cex.axis=1.75)
mtext(side=2, line=3.2, expression(bold("Effective Population Size x 10"^4)),col="black", font=2,cex=1.75)
mtext(side=3, line=1.5, "C",col="black", at=1,font=2,cex=1.75)
box()
legend("topleft", inset=0.3, legend=c("Stairway Plot Unmasked","Stairway Plot Masked"), fill=c("red","blue"),cex=1.5,text.font=2)

dev.off()
